...
## Milestone 9 — Local YouTube post generator (3h)

Deliver:

- an Editor **YouTube post** workspace seeded from project name, first hook
  caption and the explicitly applied clip-discovery moment;
- a creator-confirmed game, format, moment type, factual content summary,
  primary search phrase, supporting keywords and call to action;
- three distinct editable titles and one editable description generated by a
  pure local TypeScript service;
- 100-character title and 5,000-character description limits, primary-phrase
  placement checks and no more than three relevant hashtags;
- title, description and combined clipboard actions with a manual-copy fallback.

Exit:

- generated copy contains the creator's primary phrase in the title and opening
  description, remains within YouTube limits and never invents missing content;
- generation, editing and copying require no connected channel, network request,
  media upload, transcript or project schema change;
